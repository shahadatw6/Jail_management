...
public class SearchPrisonerSceneController implements Initializable {
    ...
    private void SearchByPrisonerIDOnClick(MouseEvent event) {
        ...
        optionalOutputLabel.setText(Integer.toString(p.getNid()));
        nameTextField.setText(p.getName());
        dobDatePicker.setValue(p.getDateOfBirth());
        genderTextField.setText(p.getGender());
        prisoncellTextField.setText(p.getPrisonCellBlock());
        statusTextField.setText(p.getStatus());
        setAllDisable();
    }
    ...
    private void SearchByNIDOnClick(MouseEvent event) {
        p = Jailor.getPrisonerByNID(Integer.parseInt(NIDTextField.getText()));
        if(p == null) {
            Jailor.showAlert("Prisoner Not Found !");
            return;
        }
        optionallabel.setText("Prisoner ID: ");
        optionalOutputLabel.setText(Integer.toString(p.getPrisoner_id()));
        nameTextField.setText(p.getName());
        dobDatePicker.setValue(p.getDateOfBirth());
        genderTextField.setText(p.getGender());
        prisoncellTextField.setText(p.getPrisonCellBlock());
    ...
    private void EditOrSaveButtonOnClick(MouseEvent event) {
        if(editable == false) {
            ...
            Jailor.update(p, StatusCombox.getValue(), prisonecellComboBox.getValue());
            setAllDisable();
            editable = false;
...
//        prisonecellComboBox.getItems().addAll(
//            "Cell Block 1",
//            "Cell Block 2",
//            "Cell Block 3",
//            "Cell Block 4",
